...
public class Trie {
  ...
  private TNode root= new TNode((char)0);

  /**
   *  Ex-> abc
   *  Take a temp crawlNode initialized with root trieNode
   *  check if root's children map contains 'a'
   *   if not contain
   *        create a new trieNode with value a and put in root's children map <a,new trieNode>
   *   if contains
   *        get key(a) value -> trieNode, set it to crawlNode
   *
   *   and keep repeating until all abc feeded.
   *   set last character trienode isEnd value -> true
   */
  ...
  public boolean wordExist(String input){
    TNode crawl= root;
    for(char ch:input.toCharArray()){
      Map<Character,TNode> child = crawl.getChildren();
...
*/
   ...
   *    2. if exist there are two possibilities:
   *    last character's isEnd can be true/false
   *    if we want to delete ban in two cases below
   *    Ex -> 1)  b->a -> t
   *                \         this case isEnd -> true
   *                 n
   *          2) b -> a -> n -> d
   *                          in this case isEnd -> false
   *    in second case we're not going to delete ban
   *    in first case we can delete only n to delete ban
   *
   *    step 1: while traversing whether word exist or not
   *        we'll add each character in basics.stack
   *     step 2: if last character contains isEnd -> true
   *        we'll pop trieNode until it's children's size is 1 or it's children is empty(for last end node)
   *     so, here in ban  n will be deleted but a's children size is 2, here it breaks
   *
   */
  public boolean deleteWord(TNode root, String input){
    TNode crawl=root;
    Stack<TNode> nodeKeeper = new Stack<>();
    for(char ch:input.toCharArray()){
      Map<Character,TNode> children= crawl.getChildren();
      if(children.containsKey(ch)){
