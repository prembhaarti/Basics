...
public class FindDuplicate {
  ...
  /**
   * arr={1,2,3,3,4,4,5};
   * given basics.array containing 1 to N no., find duplicate numbers
   * output: 3,4
   * O(n)
  ...
  /**
   *
   *given basics.array containing 1 to N no.
   * arr={1,2,3,3,4,4,5};
   * given basics.array containing 1 to N no., find duplicate numbers
   *
   * O(n)
   * adding basics.array size to element position and another loop to check
   */
  public void findDuplicateWithoutExtraSpace(int[] arr){
    int size=arr.length;
    //add size to each
    for(int i=0;i<arr.length;i++){
      arr[arr[i]%size]+=size;  //multiple adding will give basics.array out of bound so need to add at remainder
    }
