...
public class CelebrityProblem {
        ...
        static int findCelebrity(int n)
        {
            ...
            // Step 1 :Push everybody onto basics.stack
            for (int i = 0; i < n; i++)
            {
                st.push(i);
            }
            ...
            while (st.size() > 1)
            {
                ...
                // basics.stack, discard one person based on return
                // status of knows(A, B).
                int a = st.pop();
                int b = st.pop();

                // Step 3 : Push the remained person onto basics.stack.
                if (knows(a, b))
                {
                    st.push(b);
                }
