...
import basics.disjointset.DisjointS;
import basics.disjointset.DsNode;

/**
 * Kruskal algorithm is used to find out the minimum spanning basics.tree
 *
 * Disjoint set data structure can be used easily for this algorithm
 *
 * 1. Sort all edges as per cost
 * 2. use makeSet of disjoint set and feed each vertex from the basics.graph.
 ...
 * 5. from output of edge result set connect each edge you'll find the minimum spanning basics.tree.
 *
 *
 * key is minimum spanning basics.tree-> we have sorted all edges, so minimum first added
