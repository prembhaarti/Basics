...
/**
 * @author lovebharti
 * Graph is traversed using DFS using basics.stack
 * Here recursion helping in basics.stack behavior.
 *
 ...
 * 	if it's visited then it's cyclic basics.graph
 */
public class DFS<T> {
			...
			if(vertices!=null && vertices.size()>0){
				for(GNode<T> node:vertices){
					...
					if(!node.isVisited()){
						...
						System.out.println("Graph is having cycle @ basics.tree.Node:"+node.getData());
						break;
					}
				}
			}
