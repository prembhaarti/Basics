...
public class CoinChange {
   ...
   */
	...
	public int[][] createMatrix(int coins[],int totalSum){
	 ...
	 */
	public void bottomUp(int[][] resultMatrix,int sumValue,int[]coins){
		int i=sumValue;
		int j=coins.length;
		while(i!=0) {
	...
	public static void main(String[] args) {
		...
		int matrix[][]=cc.createMatrix(cc.coins, cc.totalSum);
		for(int i=1;i<=cc.totalSum;i++) {
			System.out.println("\nTotal coins required to make " + i + " rupees :" + matrix[i][cc.coins.length]);
			System.out.print("Coins are:");
			cc.bottomUp(matrix, i, cc.coins);
		}
	}
}