      ...
      for(int i=1;i<floor+1;i++) {
        if (i < j) {
          ...
          for (int k = 1; k <= i; k++) {
            ...
            //a) basics.matrix[j - 1][k - 1] -> if broken now then answer was one less egg and one less floor
            //b) basics.matrix[j][i - k] -> if not broken then we got same no. of eggs but floors left i-k
