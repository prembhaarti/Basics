...
public class PalindromePartition {
   ...
   */
  /**
   * take a min cut basics.array of word length
   *
   * fill all basics.matrix by Integer.Max
   *
   *  if basics.matrix[0][wordpos]=1 means that word is palindrome already-> no cuts required
   *    cut[i] = 0
   *  else
   *    start from cut[i+1] pos to wordpos where i=0
   *      Ex- B  A  N  A <- wordPos      | B [A  N  A] -> 1 cut required
   *          i i+1
   *        check in basics.matrix[i+1][wordPos] -> ANA which will be palindrome and 1 value in basics.matrix
