...
public class GameStrategy {
   ...
   *  for more than one coin -> @ basics.matrix[i][i+j]
   *  down -> pick first -> coin[i] + down -> basics.matrix[i+1][i+j].second
   *  left -> pick last  -> coin[i+j] + left -> basics.matrix[i][i+j-1].second
   *
  ...
  public Profit getMaxProfit(int[] coins) {
    ...
    //i goes to down and i+j goes to right of basics.matrix
