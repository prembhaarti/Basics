...
/**
 * Given a directed basics.graph clone it in O(n) time where n is total number of edges
 * Test cases
...
public class CloneDirectedGraph<T> {
    ...
    public Graph<T> clone(Graph<T> graph){
        ...
        if(!graph.isDirected){
            throw new IllegalArgumentException("Cloning non directed basics.graph");
        }
        if(graph.getAllVertex().size() == 0){
            throw new IllegalArgumentException("No vertex in the basics.graph");
        }
