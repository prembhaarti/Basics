 ...
 * @author Tushar Roy
 *
 * Find all cycles in directed basics.graph using Johnson's algorithm
 *
...
public class AllCyclesInDirectedGraphJohnson {
    ...
    public List<List<Vertex<Integer>>> simpleCyles(Graph<Integer> graph) {
        ...
        while(startIndex <= graph.getAllVertex().size()) {
            ...
            //this creates basics.graph consisting of strongly connected components only and then returns the
            //least indexed vertex among all the strongly connected component basics.graph.
            //it also ignore one vertex basics.graph since it wont have any cycle.
   ...
   private Optional<Vertex<Integer>> leastIndexSCC(List<Set<Vertex<Integer>>> sccs, Graph<Integer> subGraph) {
    ...
    private boolean findCyclesInSCG(
        ...
        for (Edge<Integer> e : currentVertex.getEdges()) {
            ...
            //Store contents of basics.stack in final result.
        ...
        //remove vertex from the basics.stack.
        stack.pop();
        return foundCycle;
    }
