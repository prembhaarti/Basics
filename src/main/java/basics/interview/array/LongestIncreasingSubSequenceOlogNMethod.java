 ...
 * @author Tushar Roy
 *
 * Given an basics.array, find longest increasing subsequence in nlogn time complexity
 *
 * References
...
public class LongestIncreasingSubSequenceOlogNMethod {
    ...
    /**
     * Returns index in ToString for ceiling of s
     */
    ...
    public int longestIncreasingSubSequence(int input[]){
        ...
        for(int i=1; i < input.length; i++){
            if(input[T[0]] > input[i]){ //if input[i] is less than 0th value of ToString then replace it there.
                T[0] = i;
            }else if(input[T[len]] < input[i]){ //if input[i] is greater than last value of ToString then append it in ToString
                len++;
                T[len] = i;
                R[T[len]] = T[len-1];
