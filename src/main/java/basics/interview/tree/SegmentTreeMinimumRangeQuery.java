...
import basics.interview.bits.NextPowerOf2;

/**
 * Date 08/22/2015
 * @author Tushar Roy
 * 
 * A segment basics.tree is a basics.tree data structure for storing intervals, or segments. It allows
 * for faster querying (e.g sum or min) in these intervals. Lazy propagation is useful
 * when there are high number of updates in the input basics.array.

 * Write a program to support mininmum range query
 * createSegmentTree(int arr[]) - create segment basics.tree
 ...
 *
 * Time complexity to create segment basics.tree is O(n) since new basics.array will be at max 4n size
 * Space complexity to create segment basics.tree is O(n) since new basics.array will be at max 4n size
 * Time complexity to search in segment basics.tree is O(logn) since you would at max travel 4 depths
 * Time complexity to update in segment basics.tree is O(logn)
 * Time complexity to update range in segment basics.tree is O(range)
 * 
 * References
...
public class SegmentTreeMinimumRangeQuery {
    ...
    /**
     * Creates a new segment basics.tree based off input basics.array.
     */
    public int[] createSegmentTree(int input[]){
        NextPowerOf2 np2 = new NextPowerOf2();
        //if input len is pow of then size of 
        //segment basics.tree is 2*len -1 otherwisze
        //size of segment basics.tree is next (pow of 2 for len) * 2 -1
    ...
    /**
     * Updates segment basics.tree for certain index by given delta
     */
    ...
    /**
     * Updates segment basics.tree for given range by given delta
     */
    ...
    private void updateSegmentTree(int segmentTree[], int index, int delta, int low, int high, int pos){
        ...
        //that value in segment basics.tree at pos
        if(low == high){
            segmentTree[pos] += delta;
            return;
        }
        //otherwise keep going left and right to find index to be updated 
        //and then update current basics.tree position if min of left or right has
        //changed.
        int mid = (low + high)/2;
    ...
    private void updateSegmentTreeRangeLazy(int segmentTree[],
        ...
        //make sure all propagation is done at pos. If not update basics.tree
