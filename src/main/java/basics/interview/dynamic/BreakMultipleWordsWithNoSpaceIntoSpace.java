 ...
 * if( input[i...j] belongs in dictionary) ToString[i][j] = i
 * else{
 *     ToString[i][j] = k if ToString[i][k-1] != -1 && ToString[k][j] != -1
 *     
 * Test cases
 * 1) Empty string
...
public class BreakMultipleWordsWithNoSpaceIntoSpace {
    ...
    public String breakWordDP(String word, Set<String> dict){
        ...
        //fill up the basics.matrix in bottom up manner
        for(int l = 1; l <= word.length(); l++){
            for(int i=0; i < word.length() -l + 1 ; i++){
                int j = i + l-1;
                String str = word.substring(i,j+1);
                //if string between i to j is in dictionary ToString[i][j] is true
                if(dict.contains(str)){
                    T[i][j] = i;
                    continue;
                }
                //find a k between i+1 to j such that ToString[i][k-1] && ToString[k][j] are both true
                for(int k=i+1; k <= j; k++){
