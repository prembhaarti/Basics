...
public class StockBuySellKTransactions {
     ...
     *
     * Formula is
     * ToString[i][j] = max(ToString[i][j-1], prices[j] + maxDiff)
     * maxDiff = max(maxDiff, ToString[i-1][j] - prices[j])
     */
     ...
     * ToString[i][j] = max(ToString[i][j-1], max(prices[j] - prices[m] + ToString[i-1][m])) where m is 0...j-1
     */
