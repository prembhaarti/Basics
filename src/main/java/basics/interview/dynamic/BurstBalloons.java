 ...
 * by basics.array nums. You are asked to burst all the balloons. If the you burst balloon i you will
        ...
        for (int len = 1; len <= nums.length; len++) {
            for (int i = 0; i <= nums.length - len; i++) {
                ...
                for (int k = i; k <= j; k++) {
                    ...
                    //stay 0 otherwise it gets value ToString[i][k-1]
                    //after is similarly 0 initially. if k is j then after will
                    //stay 0 other will get value ToString[k+1][j]
                    int before = 0;
                    int after = 0;
                    if (i != k) {
                        before = T[i][k-1];
