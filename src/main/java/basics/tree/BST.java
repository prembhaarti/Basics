...
public class BST {
	...
	public boolean isMirrorTree(Node first,Node second){
		...
		//if both basics.tree's root are null they are mirrored
		if(first==null && second==null) return true;
		//if node of one basics.tree is null and corresponding other basics.tree's node is not null
		// they are not mirrored
			...
			// take one basics.tree's left node and second's basics.tree right node
...
*/
	 ...
	 *
	 * print element data when you pop element from basics.stack.
	 */
   ...
   */
	...
	public void inOrderIteration(Node root){
		...
		while(!stack.isEmpty()) {
			if (root != null) {
				stack.push(root);//push element in basics.stack
   ...
   */
	...
	public Node getSuccessor(Node root, Node successor, int data) {
		if (root != null){
			if (root.data == data) {
				// if right is not null find smallest on  left of left sub basics.tree
				if (root.rc != null) {
					successor = smallestNumberNode(root.rc);
				}
	...
	/**
	 * Q: can given basics.array in pre-order belongs to any BST
	 *
	 * #Note: whenever go to right update root, coz you are going to get the larger value.
	 *		  current basics.array element should be less than root value.
	 *
	 * take basics.array
	 ...
	 * if basics.stack not empty and present element is greater than top of basics.stack
	 * 		change root to current element and pop to element
	 *
	 * push every element to basics.stack.
	 *
	 */
	...
	/**
	 * same as level order traversal just keep putting each level in basics.stack
	 * and do traversal from right to left, so that basics.stack will give you left to right
	 */
