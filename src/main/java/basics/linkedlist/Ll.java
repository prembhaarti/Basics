...
public class Ll {
   ...
   *
   * now start to pop from both basics.stack until any of them get empty
   *
   *  get data from both basics.stack -> data1, data2 and keep a carry=0
   *  sum = data1+data2+carry
   *  if sum exceeds 9 -> set to carry to 1
   *  and sum sum=sum%10
   *
   *  and keep adding to basics.linkedlist
   *
   */
	...
	/**
	 * feed all nodes in a basics.stack.
	 * start to traverse from first node.
	 * pop node from basics.stack and check if matches with node
...
/*
		...
		ll.display(rhead);
*/
		//System.out.println("Middle basics.tree.Node:"+ll.getMiddleNode(head).data);
...
/*
		...
		System.out.println("--");
		//making two intersect basics.linkedlist
		*/
/**
 *
 *            1 -- 2
 ...
 *//*
		...
		//intersected basics.linkedlist creation end.
