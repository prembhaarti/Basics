...
public class StreamsDemo {
    ...
    private static final Predicate<Customer> notEmpty= (customer)-> null!=customer.getName() && customer.getName().length()>0;
    private static final Predicate<Customer> empty= (customer)-> null==customer.getName() || customer.getName().length()==0;
...
//        System.out.println(demo.filterEmptyNames(customers));
    ...
    public List<Customer> mapEmptyByDefaultValue(List<Customer> names){
         return names.stream()
                .filter(empty)
                .map(customer ->  new Customer(customer.getId(),"Anonymous"))
                .collect(Collectors.toList());
    }
